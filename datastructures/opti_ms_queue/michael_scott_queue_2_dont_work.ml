...
let rec push tail old_tail new_node new_tail backoff =
  match Atomic.get old_tail with
  | Nil ->
      if Atomic.compare_and_set old_tail Nil new_node then
        fix_tail tail new_tail
      else
        let backoff = Backoff.once backoff in
        push tail old_tail new_node new_tail backoff
  | Next (_, next) as new_node ->
      push tail next new_node new_tail (Backoff.once backoff)
...
let push_exn t value =
  ...
  if Atomic.compare_and_set old_tail Nil new_node then
    Atomic.compare_and_set t.tail old_tail new_tail |> ignore
  else push t.tail old_tail new_node new_tail Backoff.default
