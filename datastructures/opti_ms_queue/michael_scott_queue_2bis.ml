...
let push_exn q value =
  let rec find_tail_and_enq curr_end node backoff =
    if not (Atomic.compare_and_set curr_end Nil node) then
      match Atomic.get curr_end with
      | Nil -> find_tail_and_enq curr_end node (Backoff.once backoff)
      | Next (_, n) -> find_tail_and_enq n node Backoff.default
  in
  ...
  if not (Atomic.compare_and_set old_tail Nil newnode) then begin
    ...
    | Nil -> find_tail_and_enq old_tail newnode Backoff.default
    | Next (_, n) -> find_tail_and_enq n newnode Backoff.default
  end;
  if not (Atomic.compare_and_set q.tail old_tail new_tail) then
    fix_tail q.tail new_tail
